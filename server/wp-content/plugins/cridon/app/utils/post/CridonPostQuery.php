 ...
 * Normally, I can inherit this class with WP_Query but I don't know if in WP upgrade I must implement more functions in parent object
 * ( if WP developper choose to implement Interface in object WP_Query )
 */
class CridonPostQuery {
    ...
    private function isPostData( $data ){
        if( is_array( $data ) && !empty( $data ) ){
            if( isset( $data[0]->post ) && ( $data[0]->post instanceof WP_Post ) ){
                return 'array';                
            }
        }else{
    ...
    private function initializePostData( $data ){
        ...
        $this->wp_query->current_post = -2;//It's use WP increment loop
    }
    /**

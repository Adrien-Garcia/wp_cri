...
class AdminPostHelper extends MvcHelper
{
    ...
    public function admin_actions_cell($controller, $object)
    {
        ...
        $links[] = '<a href="' . $url . '" title="'.Config::$actionsWpmvcTranslation['view'].' ' . $encoded_object_name . '">'.Config::$actionsWpmvcTranslation['view'].'</a>';        $links[]             = '<a href="' . $deleteLink . '" title="'.Config::$actionsWpmvcTranslation['delete'].' ' . $encoded_object_name . '" onclick="return confirm(&#039;'.Config::$msgConfirmDelete.' ' . $encoded_object_name . '?&#039;);">'.Config::$actionsWpmvcTranslation['delete'].'</a>';
        $html                = implode(' | ', $links);

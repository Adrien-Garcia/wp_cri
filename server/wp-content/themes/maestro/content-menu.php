<ul id="menu-menu-principal" class="nav top-nav clearfix">
	<li id="menu-item-984" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children">
		<a href="#"><span>Accèder</span> aux bases de connaissances</a>
		<ul class="sub-menu">
			<li id="menu-item-1530" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children">
				<a>Ma veille juridique</a>
				<ul class="sub-menu">
					<li id="menu-item-1346" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Culpa eu ad eiusmod.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum id Excepteur occaecat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
					<li id="" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Lorem ipsum Mollit cupidatat.</a>
					</li>
				</ul>
			</li>
			<li id="menu-item-1531" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="#">Flash infos</a>				
			</li>
			<li id="menu-item-1531" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="#">Base des consultations du cridon</a>				
			</li>
			<li id="menu-item-1531" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="#">Cahiers du cridon</a>				
			</li>
			<li id="menu-item-1531" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="#">Sites partenaires utiles</a>				
			</li>
			<li>
				<div class="block rechercher js-home-block-link">
					<div class="content">
						<h2>
							<?php _e('Rechercher'); ?>
							<span><?php _e('dans les bases de connaissances'); ?></span> 
						</h2>
						<a href="">+</a>
					</div>						
				</div>
			</li>
		</ul>
	</li>
	<li id="menu-item-1173" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-has-children">
		<a href="#"><span>Commander</span> un service</a>
		<ul class="sub-menu">
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children">
				<a>Poser une question</a>
				<ul class="sub-menu">
					<li id="menu-item-1384" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Urgente</a>
					</li>
					<li id="menu-item-1385" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Semaine</a>
					</li>
					<li id="menu-item-1383" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Normal</a>
					</li>
					<li id="menu-item-1387" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Demande de documentation</a>
					</li>
					<li id="menu-item-2241" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Prendre un rendez-vous</a>
					</li>
				</ul>
			</li>
			<li id="menu-item-1534" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children">
				<a>Se former</a>
				<ul class="sub-menu">
					<li id="menu-item-1389" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Catalogue</a>
					</li>
					<li id="menu-item-1390" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Me former en ligne</a>
					</li>
					<li id="menu-item-1390" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Me former en temps réel</a>
					</li>
				</ul>
			</li>
			<li id="menu-item-1535" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children">
				<a>Service +</a>
				<ul class="sub-menu">
					<li id="menu-item-1395" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Traduction</a>
					</li>
					<li id="menu-item-1393" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Rèseau partenaires étrangers</a>
					</li>
					<li id="menu-item-1392" class="menu-item menu-item-type-post_type menu-item-object-page">
						<a href="#">Assistance fiscale</a>
					</li>
				</ul>
			</li>
			<li id="menu-item-1535" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="#">Calculer</a></li>
			<li>
				<div class="block acceder js-home-block-link">
					<div class="content">
						<h2>
							<?php _e('Accéder'); ?>
							<span><?php _e('à ma veille juridique'); ?></span>
						</h2>
						<a href="<?php echo MvcRouter::public_url(array('controller' => 'veilles', 'action'     => 'index')) ?>">+</a>
					</div>						
				</div>
			</li>
		</ul>
	</li>
	<li id="menu-item-1173" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-has-children">
		<a href="#"><span>Connaître</span> le cridon</a>
		<ul class="sub-menu">
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Qui sommes nous ?</a>				
			</li>
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Offre de services</a>				
			</li>
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Actu Cridon</a>				
			</li>
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Partenaires</a>				
			</li>
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Dirigeants / collaborateurs</a>				
			</li>
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Chiffres clés</a>				
			</li>
			<li id="menu-item-1533" class="menu-item menu-item-type-custom menu-item-object-custom">
				<a href="">Contacts</a>				
			</li>
			<li>
				<div class="block consulter layer-posez-question_open js-question-open" >
					<div class="content">
						<h2>
							<?php _e('Consulter'); ?>
							<span><?php _e('un expert juridique'); ?></span>
						</h2>
						<a class="poser-question layer-posez-question_open js-question-open" href="#" >+</a>
					</div>						
				</div>
			</li>
		</ul>
	</li>
</ul>
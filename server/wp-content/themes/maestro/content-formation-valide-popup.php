<div id="layer-valide-formation" class="popup js-formation-formulaire-popup" style="display: none;">
    ...
    <div class="block">
        <?php if (!empty($preinscription)) : ?>
            <?php _e('Votre demande de pré-inscription a bien été envoyée, vous serez recontacté par le CRIDON LYON prochainement'); ?>
        <?php elseif (!empty($demandeFormation) || !empty($demandeGenerique)) : ?>
            <?php _e('Votre demande de formation a bien été envoyée, vous serez recontacté par le CRIDON LYON prochainement'); ?>
        <?php endif; ?>
        <br />

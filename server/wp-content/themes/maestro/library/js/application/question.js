...
App.Question = {
    ...
    addListeners: function () {
        ...
        this.$buttonQuestionMaQuestion.on('click', function () {
    ...
    eventFileReset: function (reset) {
        ...
        if (typeof reset !== 'undefined') {
            file = reset.siblings(this.fileQuestionSelector);
    ...
    eventZoneQuestionSupportClick: function (zone) {
        ...
        radio = zone.find(this.radioQuestionSupportSelector).first();
    ...
    eventZoneQuestionExpertiseClick: function (zone) {
        ...
        zone.addClass(this.selectedClass);
        radio = zone.find(this.radioQuestionExpertiseSelector).first();
        radio.prop('checked', true).change();

        // Get the id of expertise
        id = radio.val();
        label = radio.data('label');
        ...
        items = $(this.itemQuestionExpertiseSelector).detach();
        ...
        newItems = $(this.inputQuestionExpertiseNSelector + id).detach();

        // Order supports by data-order
        ordered = [];
        for (i = 0; i < newItems.length; i++) {
            $i = $(newItems.get(i));
            order = parseInt($i.data('order'), 10);
            while (typeof ordered[order] !== 'undefined') {
                order++;
            }
            ordered[order] = $i;
        }
    ...
    eventButtonSupportClick: function (button) {
        ...
        if (typeof support === 'undefined') {
            for (i = 0; i < button[0].classList.length; i++) {
                // button[0].classList.item(i);
                re = new RegExp(this.buttonQuestionSupportNSelector + '(\\d+)');
                match = re.exec(button[0].classList.item(i));
                if (match && match[1]) {
                    support = match[1];
                    break;
                }
    ...
    eventSubmitQuestion: function () {
        ...
        if (nbFiles > 0) {
            this.$fileQuestion.each(function () {
                ...
                if (file) {
                    if (formdata && (parseInt(file.size, 10) <= parseInt(jsvar.question_max_file_size, 10))) {
    ...
    successQuestion: function (_data) {
        var data = JSON.parse(_data);
        // show message response
        var content = $(document.createElement('ul'));
        if (typeof data !== 'undefined' && typeof data.error !== 'undefined' && Array.isArray(data.error)) {
